...
rda_tree <- rda(comm4 ~ as.factor(env_species_counts$year) + as.factor(env_species_counts$block.y) + as.factor(env_species_counts$site.y) + as.factor(env_species_counts$site_type) + env_species_counts$spp_counts + env_species_counts$mean_dbh + env_species_counts$canopy_cover , data=env_species_counts)
rda_tree
...
# it doesn't seem that vegetation dbh or dist means, are revealing any signal
summary(aov(bird_div_S_a ~ year + block.y + mean_dbh + mean_dist + site_type + canopy_cover + midstory_cover, data = env_matrix))

# creating the same model with JUST wetlands

canopy_model <- aov(log(bird_div_S_PIE_a) ~ canopy_cover * canopy_dom, data = env_matrix_wet)
...
ggplot(data = env_matrix, mapping = aes(mean_dist, bird_div_S_PIE_a), colour = site_type) + geom_point() + geom_smooth(method = lm)
...
ggplot(data = env_matrix_wet, mapping = aes(canopy_dom, bird_div_S_a)) + geom_boxplot() + theme_bw()
```

...
dat_all <- read.csv("~/ephem_birds_data_analysis/wetland_birds/raw_data - all_bird_data.csv")

# env matrix
bird_env <- read.csv("~/ephem_birds_data_analysis/wetland_birds/raw_data - veg_2022.csv")
...
comm3 <- with(super_bird_matrix, tapply(total, list(id3, species), sum))
...
env <- super_bird_matrix %>% select('wetland_id', 'block.y', 'date', 'year', 'spp', 'dbh', 'dist', 'site.y', 'site_type.x')
...
env_species_counts <- env %>% group_by(wetland_id, site.y, year, date, block.y) %>% summarise(
...
rownames(env_species_counts) <- rownames(comm3)

## do some vegan analysis

rda_tree <- rda(comm3 ~ as.factor(env_species_counts$year) + as.factor(env_species_counts$block.y) + as.factor(env_species_counts$site.y) + as.factor(env_species_counts$site_type) + env_species_counts$spp_counts + env_species_counts$mean_dbh + env_species_counts$mean_dist , data=env_species_counts)
rda_tree
